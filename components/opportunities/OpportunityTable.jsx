...
function OpportunityStatus({ opportunity }) {
  if (opportunity.archived) {
    return <span className={styles.lifecycleBadge}>Archived</span>;
  }
  if (isExpiredOpportunity(opportunity)) {
    return <span className={`${styles.lifecycleBadge} ${styles.expiredBadge}`}>Expired</span>;
  }
  return null;
}
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {opportunities.map((opportunity) => (
              <tr
                key={opportunity.id}
                className={`${styles.row}${opportunity.archived ? ` ${styles.rowArchived}` : ""}`}
              ...
              >
                ...
                <td className={styles.organizationCell}>
                  {opportunity.organization || <span className={styles.muted}>—</span>}
                </td>
                <td className={styles.deadlineCell}>
                  <div className={styles.deadline}>
                    <span className={!opportunity.deadline ? styles.muted : undefined}>{formatDateOnly(opportunity.deadline)}</span>
                  </div>
                </td>
                <td className={styles.requirementsCell}>
                  {opportunity.requirements ? (
                    <span className={styles.clamp}>{opportunity.requirements}</span>
                  ) : (
      ...
      <div className={styles.mobileList}>
        {opportunities.map((opportunity) => (
          <button
            key={opportunity.id}
            type="button"
            className={`${styles.mobileCard}${opportunity.archived ? ` ${styles.mobileCardArchived}` : ""}`}
            onClick={() => onSelect(opportunity)}
          >
            <div className={styles.mobileCardHeader}>
              <span className={styles.mobileCardTitle}>{opportunity.title}</span>
              <OpportunityStatus opportunity={opportunity} />
            </div>
            <div className={styles.mobileCardMeta}>
              ...
              <span>·</span>
              <span>{formatDateOnly(opportunity.deadline)}</span>
            </div>
            {opportunity.requirements ? (
              <p className={styles.mobileRequirements}>{opportunity.requirements}</p>
            ) : null}
          </button>
        ))}
      </div>
