...
function RequirementNodeEditor({ node, depth = 0, disabled, onReplace, onRemove }) {
  if (node.kind === "group") {
    ...
    return <section style={{ border: "1px dashed var(--fs-app-border-raised)", borderRadius: 10, padding: ".75rem", marginLeft: depth ? ".7rem" : 0, display: "grid", gap: ".6rem" }}>
  ...
  return <section style={{ border: "1px solid var(--fs-app-border)", borderRadius: 10, padding: ".75rem", marginLeft: depth ? ".7rem" : 0, display: "grid", gap: ".6rem" }}>
...
export default function OpportunityRequirementsEditor({ standardizedRequirements = [], miscRequirements = "", disabled = false, onChange }) {
  ...
  return <>
    ...
    {rawRequirementsText ? <div className={styles.fieldFull}><details><summary style={{ color: "var(--fs-app-text-muted)", cursor: "pointer", fontSize: ".76rem", fontWeight: 700 }}>Original source requirements</summary><p style={{ margin: ".45rem 0 0", whiteSpace: "pre-wrap", color: "var(--fs-app-text-muted)", fontSize: ".76rem", lineHeight: 1.45 }}>{rawRequirementsText}</p></details></div> : null}
  </>;
}
