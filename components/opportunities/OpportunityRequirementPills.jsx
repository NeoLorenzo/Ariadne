...
import { formatOpportunityRequirement, normalizeStandardizedRequirements } from "@/lib/opportunities/opportunityRequirements";
...
export default function OpportunityRequirementPills({ opportunity, assessments = [], editable = false, onSetAssessment, onClearAssessment, showOverall = false }) {
  const requirements = useMemo(() => normalizeStandardizedRequirements(opportunity?.standardizedRequirements ?? opportunity?.standardized_requirements), [opportunity]);
  ...
  const renderPill = (requirement) => {
    ...
    if (!editable) return <span key={requirement.id} className={`${styles.requirementPill} ${styles[`requirementPill_${status}`]}`} title={title}>{pillLabel}</span>;
