...
import OpportunityCandidateEditor from "@/components/opportunities/OpportunityCandidateEditor";
import OpportunityCandidateTable from "@/components/opportunities/OpportunityCandidateTable";
import {
...
import candidateStyles from "@/components/opportunities/OpportunityCandidate.module.css";
...
export default function OpportunityCandidatesPage() {
  ...
  const saveCandidate = async (form) => {
    ...
    try {
      if (selectedCandidate?.id) {
        await updateOpportunityCandidate({
  ...
  return (
    <AppShell activeNavItem="opportunity-candidates" hideMobileNav={isEditorOpen}>
      <section className={styles.workspace}>
        <section className={styles.panel}>
          ...
          <div className={candidateStyles.surfaceTabs} aria-label="Opportunity Landscape views">
            <Link className={candidateStyles.surfaceTab} href="/opportunities">Landscape</Link>
            <span className={`${candidateStyles.surfaceTab} ${candidateStyles.surfaceTabActive}`}>Review inbox</span>
          </div>
          ...
          <div className={styles.filters}>
            ...
            <select className={styles.control} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Filter review status">
              ...
              {OPPORTUNITY_CANDIDATE_REVIEW_STATUSES.map((status) => (
                <option key={status} value={status}>{OPPORTUNITY_CANDIDATE_REVIEW_LABELS[status]}</option>
              ))}
            </select>
          </div>
