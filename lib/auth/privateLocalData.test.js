...
describe("private local data cleanup", () => {
  ...
  it("clears Opportunity Landscape caches, pending operations, and candidate inbox", () => {
    ...
    expect(removeItem).toHaveBeenCalledWith("ariadne_opportunity_applications_v1");
  ...
  it("clears the current Directions cache", () => {
    const removeItem = installLocalStorage();

    clearLocalPrivateData();

    expect(removeItem).toHaveBeenCalledWith("ariadne_directions_v2");
  });
