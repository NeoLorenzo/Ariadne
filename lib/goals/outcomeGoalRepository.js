...
import { hasLocalGoalTask, removeGoalTask, syncGoalTask, syncLocalGoalTask } from "@/lib/tasks/goalTaskSync";
...
export async function saveOutcomeGoal({ goals, goal, objectiveId, userId, revisionReason, onLocalUpdate }) {
  ...
  if (supabase && userId) {
    ...
    const { error } = previous
      ? await supabase.rpc("update_outcome_goal_semantic", {
        goal_id: nextGoal.id,
  ...
  if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent("ariadne:outcome-goals-changed", { detail: { objectiveId: targetObjectiveId } }));
  return next;
}
