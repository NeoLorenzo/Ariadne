...
export function linkedTaskId(goalId) {
...
function writeLocalTasks(tasks) {
  ...
  window.dispatchEvent(new CustomEvent("ariadne:tasks-changed"));
}
...
export function updateLinkedTask(tasks, goal, enabled) {
...
export async function syncCloudTasks(userId, mutate) {
...
export function syncLocalGoalTask(goal) {
  const mutate = (tasks) => updateLinkedTask(tasks, goal, Boolean(goal.displayOnTodoList));
