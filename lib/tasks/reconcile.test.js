...
describe("task synchronization reconciliation", () => {
  ...
  it("retains target dates as distinct task planning metadata", () => {
    const [normalizedMissing] = sanitizeTaskList([task("missing", "Missing target", 1)]);
    expect(normalizedMissing.targetDate).toBe("");

    const [normalized] = sanitizeTaskList([task("target", "Targeted", 2, {
      dueDate: "2026-10-20",
  ...
  it("retains rich task data and invalid input is safe", () => {
    ...
    expect(result.targetDate).toBe("2026-08-28");
