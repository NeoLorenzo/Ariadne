...
describe("strategic objective reconciliation", () => {
  ...
  it("allows a fourth active objective under the multidirectional model", async () => {
    const existing = [0, 1, 2].map((position) => ({
    ...
    installBrowserState(existing);
    cloud.rpc.mockResolvedValue({ data: { updated_at: "2026-09-07T00:30:00.000Z" }, error: null });

    const next = await saveStrategicObjective({
      objectives: existing,
      objective: { title: "Fourth", description: "", successCondition: "Done", status: "active" },
      directionId: "direction-1",
      userId: "owner-1"
    });

    expect(next.filter((item) => item.status === "active")).toHaveLength(4);
    expect(next.at(-1)).toMatchObject({ title: "Fourth", position: 3 });
  });
});
