...
describe("buildGitHubOAuthCallbackUrl", () => {
  it("returns the Dashboard root for normal and base-path deployments", () => {
    expect(buildGitHubOAuthCallbackUrl("https://example.test", "")).toBe("https://example.test/");
    expect(buildGitHubOAuthCallbackUrl("https://example.test/", "/Ariadne/")).toBe(
      "https://example.test/Ariadne/"
    );
  });
});
