...
describe("opportunity candidate repository", () => {
  ...
  it("routes ingestion through the canonical RPC and preserves reviewed state returned by the server", async () => {
    ...
    const result = await ingestOpportunityCandidate({
      candidate: {
        ...
        standardizedRequirements: [{ id: "requirement-1", kind: "experience" }],
        miscRequirements: "Research experience"
      },
      userId: "owner-1",
    ...
    expect(cloud.rpc).toHaveBeenCalledWith(
      "ingest_opportunity_candidate",
      expect.objectContaining({
        p_user_id: "owner-1",
        p_source_name: reviewed.sourceName,
        p_source_external_id: reviewed.sourceExternalId,
        p_last_seen_at: "2026-09-10T09:00:00.000Z",
        p_misc_requirements: "Research experience",
        p_standardized_requirements: expect.any(Array)
      })
    );
    expect(cloud.from).not.toHaveBeenCalled();
  });

  it("caches a newly created candidate returned by the canonical ingestion RPC", async () => {
    const values = installBrowserState();
    const candidate = sampleCandidate({
      id: "opportunity-candidate-new",
      standardizedRequirements: []
    });
    cloud.rpc.mockResolvedValue({
      data: {
        candidate: toRow(candidate),
        created: true,
    ...
    const result = await ingestOpportunityCandidate({
      candidate,
      userId: "owner-1",
      now: new Date(candidate.lastSeenAt)
    });

    expect(result.created).toBe(true);
    expect(result.duplicateReason).toBeNull();
    expect(JSON.parse(values.get(OPPORTUNITY_CANDIDATE_STORAGE_KEY)).candidates).toEqual([
      expect.objectContaining({ id: "opportunity-candidate-new" })
    ]);
  });
});
