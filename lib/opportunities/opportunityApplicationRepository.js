...
export const OPPORTUNITY_APPLICATION_SYNC_PENDING = "OPPORTUNITY_APPLICATION_SYNC_PENDING";
...
function makeSyncError(cause) {
  const error = new Error(OPPORTUNITY_APPLICATION_SYNC_PENDING);
  error.cause = cause;
  return error;
}
...
export async function createOpportunityApplication({ application, userId, onLocalUpdate }) {
  ...
  if (!supabase || !userId) throw makeSyncError(new Error("SUPABASE_UNAVAILABLE"));
  const { data, error } = await supabase
    .from("opportunity_applications")
  ...
  if (error) throw makeSyncError(error);
