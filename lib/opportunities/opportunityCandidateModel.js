...
export const OPPORTUNITY_CANDIDATE_SOURCE_TYPES = [
  "manual",
  "agent",
  "scraper",
  "api",
...
export const OPPORTUNITY_CANDIDATE_SOURCE_LABELS = {
  manual: "Manual",
  agent: "Agent",
  scraper: "Scraper",
  api: "API",
...
export const OPPORTUNITY_CANDIDATE_REVIEW_STATUSES = [
  "pending",
  "accepted",
  "rejected",
  "duplicate"
...
export const OPPORTUNITY_CANDIDATE_REVIEW_LABELS = {
  pending: "Pending",
  accepted: "Accepted",
  rejected: "Rejected",
  duplicate: "Duplicate"
...
export function makeOpportunityCandidateFingerprint(input = {}) {
  ...
  const identity = [
    ...
    normalizeKey(normalized.requirements),
    normalized.deadline,
...
export function normalizeOpportunityCandidate(input = {}) {
  ...
  return {
    ...
    requirements: asTrimmedString(input.requirements),
...
export function createOpportunityCandidateRecord(input = {}, now = new Date()) {
  ...
  return {
    ...record,
    contentHash: record.contentHash || makeOpportunityCandidateFingerprint(record)
  };
}

function isHttpUrl(value) {
  if (!asTrimmedString(value)) return true;
...
function sameNormalizedText(left, right) {
  return Boolean(normalizeKey(left)) && normalizeKey(left) === normalizeKey(right);
}
...
export function findOpportunityCandidateDuplicate(candidates = [], input = {}) {
  ...
  if (candidate.sourceExternalId) {
    const byExternalId = normalized.find((item) => item.sourceExternalId
      && item.sourceExternalId === candidate.sourceExternalId
      && normalizeKey(item.sourceType) === normalizeKey(candidate.sourceType)
      && normalizeKey(item.sourceName) === normalizeKey(candidate.sourceName));
  ...
  if (candidate.title && candidate.organization) {
    const byIdentity = normalized.find((item) => sameNormalizedText(item.title, candidate.title)
      && sameNormalizedText(item.organization, candidate.organization)
      && compatibleDates(item, candidate));
