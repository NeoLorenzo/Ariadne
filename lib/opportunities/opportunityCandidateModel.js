...
export function isOpportunityCandidateDescriptionExcerpt(input = {}) { const candidate = input && typeof input === "object" ? input : {}; const payload = normalizeSourcePayload(candidate.sourcePayload ?? candidate.source_payload); return payload.description_is_excerpt === true || normalizeKey(payload.description_completeness) === "excerpt"; }
...
export function normalizeOpportunityCandidate(input = {}) {
  ...
  const standardizedRequirements = normalizeStandardizedRequirements(input.standardizedRequirements ?? input.standardized_requirements);
  const rawRequirementsText = asTrimmedString(
    input.rawRequirementsText ?? input.raw_requirements_text ?? getRawRequirementsText(standardizedRequirements)
  );
  const applicationComponentsInput =
    input.applicationComponents ?? input.application_components ?? getApplicationComponents(standardizedRequirements);
  const applicationComponents = normalizeStandardizedRequirements(applicationComponentsInput)
    .filter((node) => node.kind === "application_component");
...
export function validateOpportunityCandidate(input = {}) {
  ...
  const rawStandardizedRequirements = input.standardizedRequirements ?? input.standardized_requirements ?? [];
  const rawApplicationComponents = input.applicationComponents ?? input.application_components ?? [];
  const integrityErrors = validateRequirementDocumentIntegrity({
    standardizedRequirements: rawStandardizedRequirements,
    applicationComponents: rawApplicationComponents
  });
  const requirementErrors = validateStandardizedRequirements(rawStandardizedRequirements);
  const combinedRequirementErrors = [...integrityErrors, ...requirementErrors];
  if (combinedRequirementErrors.length) errors.standardizedRequirements = [...new Set(combinedRequirementErrors)].join(" ");
  return errors;
}
