...
export function normalizeOpportunity(input = {}) {
  ...
  const rawRequirementsText = asTrimmedString(
    input.rawRequirementsText ?? input.raw_requirements_text ?? getRawRequirementsText(structuredRequirements)
  );
  const applicationComponentsInput =
    input.applicationComponents ?? input.application_components ?? getApplicationComponents(structuredRequirements);
