...
import {
  ...
  isOpportunityCandidateDescriptionExcerpt,
...
describe("opportunity candidate model", () => {
  ...
  it("identifies source descriptions that are explicitly marked as excerpts", () => {
    expect(isOpportunityCandidateDescriptionExcerpt({
      sourcePayload: { description_is_excerpt: true }
    })).toBe(true);
    expect(isOpportunityCandidateDescriptionExcerpt({
      source_payload: { description_completeness: "excerpt" }
    })).toBe(true);
    expect(isOpportunityCandidateDescriptionExcerpt({
      sourcePayload: { description_completeness: "full" }
    })).toBe(false);
    expect(isOpportunityCandidateDescriptionExcerpt(null)).toBe(false);
  });
  ...
  it("preserves raw requirement evidence and canonical application components", () => {
    ...
    expect(candidate.rawRequirementsText).toBe("Bachelor's degree; CV.");
    expect(candidate.applicationComponents).toEqual([
      expect.objectContaining({ id: "cv", componentType: "cv_resume" })
    ]);
  });

  it("reports malformed legacy application-component tuples as invalid", () => {
