...
function readLocalOpportunities(userId = "") {
  ...
  // Support an early array-shaped cache if one ever existed during development.
...
function enqueueSaveOperation(userId, record, { isCreate, baseUpdatedAt = "" }) {
  ...
  // Repeated local saves before synchronization collapse into the latest record while
  // preserving the original remote base (or the fact that the record is still a create).
...
export async function flushOpportunityOperations(userId) {
  if (!supabase || !userId) {
    return { ok: true, deferred: true, results: [] };
  }
  ...
  for (const operation of readPendingOperations(userId)) {
    if (operation.status === "conflict") {
      return {
        ...
        conflict: true,
        operation,
        error: makeConflictError(operation.entityId),
        results
      };
    }
...
function mergeRemoteWithLocal(local, remote) {
  ...
  // A remote collection is not allowed to silently erase a locally valid record.
  // Explicit local deletes are already represented by queued delete operations.
