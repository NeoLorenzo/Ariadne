...
function toCreateRow(opportunity, userId) {
  return {
    ...
    requirements: opportunity.miscRequirements || null,
    standardized_requirements: opportunity.standardizedRequirements || [],
    misc_requirements: opportunity.miscRequirements || null,
...
export async function flushOpportunityOperations(userId) {
  ...
  for (const operation of readPendingOperations(userId)) {
    ...
    try {
      ...
      const wrapped = error?.message === OPPORTUNITY_SYNC_CONFLICT ? error : makeRepositoryError(OPPORTUNITY_SYNC_PENDING, error);
      markPendingOperationFailed(userId, operation, wrapped);
      return { ok: false, conflict: wrapped.message === OPPORTUNITY_SYNC_CONFLICT, operation, error: wrapped, results };
    }
  }
...
export async function loadOpportunities(userId) {
  ...
  if (remote.length === 0 && localAfterFlush.length > 0) return sortOpportunitiesByDeadline(localAfterFlush);
...
export async function updateOpportunity({ opportunityId, patch, userId, onLocalUpdate }) {
  ...
  const candidate = { ...current, ...patch, id: current.id, createdAt: current.createdAt };
  ...
  const local = readLocalOpportunities(userId);
  const next = writeLocalOpportunities(userId, local.map((item) => item.id === opportunityId ? updated : item));
  notifyLocalUpdate(onLocalUpdate, next);
  enqueueSaveOperation(userId, updated, { isCreate: false, baseUpdatedAt: current.updatedAt });
...
export async function setOpportunityArchived({ opportunityId, archived, userId, onLocalUpdate }) {
  return updateOpportunity({ opportunityId, patch: { archived: Boolean(archived) }, userId, onLocalUpdate });
}
