...
function writeJson(storageKey, value) {
  ...
  try {
    ...
    // Candidate cache is a resilience layer; storage failure must not alter cloud state.
  }
}

function normalizeCollection(value) {
  return (Array.isArray(value) ? value : [])
    .map((item) => normalizeOpportunityCandidate(item))
...
function writeLocalCandidates(userId, candidates) {
  ...
  writeJson(OPPORTUNITY_CANDIDATE_STORAGE_KEY, {
    version: 1,
...
function notifyLocalUpdate(onLocalUpdate, candidates) {
  onLocalUpdate?.(candidates);
}
...
function replaceLocalCandidate(userId, candidate) {
  ...
  return writeLocalCandidates(
    userId,
    exists
      ? local.map((item) => item.id === candidate.id ? candidate : item)
      : [...local, candidate]
  );
}
...
function requireCloud(userId) {
  if (supabase && userId) return;
  throw new Error(OPPORTUNITY_CANDIDATE_CLOUD_UNAVAILABLE);
}

function throwValidationError(input) {
  const errors = validateOpportunityCandidate(input);
  if (Object.keys(errors).length === 0) return;
  const error = new Error("INVALID_OPPORTUNITY_CANDIDATE");
  error.validationErrors = errors;
  throw error;
}

function makeConflictError(candidateId) {
  const error = new Error(OPPORTUNITY_CANDIDATE_CONFLICT);
  error.details = { candidateId };
  return error;
}
...
export async function loadOpportunityCandidatesState(userId) {
  const local = readLocalCandidates(userId);
  if (!supabase || !userId) {
    return { candidates: sortOpportunityCandidates(local), cloudAvailable: false, source: "cache", error: null };
  }
  ...
  if (error) {
    return { candidates: sortOpportunityCandidates(local), cloudAvailable: false, source: "cache", error };
  }

  const remote = normalizeCollection((data || []).map(fromRow));
  if (remote.length === 0 && local.length > 0) {
    return { candidates: sortOpportunityCandidates(local), cloudAvailable: true, source: "cache-preserved", error: null };
  }
...
export async function loadOpportunityCandidates(userId) {
  const result = await loadOpportunityCandidatesState(userId);
...
function editablePatch(patch = {}) {
  return Object.fromEntries(Object.entries(patch).filter(([key]) => EDITABLE_FIELDS.has(key)));
}
...
export async function updateOpportunityCandidate({ candidateId, patch, userId, onLocalUpdate }) {
  ...
  let query = supabase
    .from("opportunity_candidates")
    .update(toEditableUpdateRow(normalized))
    .eq("id", candidateId)
    .eq("user_id", userId);
...
export async function setOpportunityCandidateReview({
  candidateId,
  reviewStatus,
  rejectionReason = "",
  userId,
  ...
  if (reviewStatus !== "rejected" && reviewStatus !== "duplicate") {
    throw new Error("INVALID_OPPORTUNITY_CANDIDATE_REVIEW_TRANSITION");
  }
...
export async function acceptOpportunityCandidate({ candidateId, candidatePatch = {}, userId, onLocalUpdate }) {
  ...
  const normalizedCandidate = normalizeOpportunityCandidate({
    ...current,
    ...editablePatch(candidatePatch)
  });
  ...
  const opportunityInput = {
    ...
    requirements: normalizedCandidate.requirements,
  ...
  if (Object.keys(opportunityErrors).length) {
    const error = new Error("INVALID_OPPORTUNITY");
    error.validationErrors = opportunityErrors;
    throw error;
  }
  ...
  const { data, error } = await supabase.rpc("accept_opportunity_candidate", {
    ...
    p_requirements: opportunity.requirements || null,
...
export async function ingestOpportunityCandidate({ candidate, userId, onLocalUpdate, now = new Date() }) {
  ...
  let query = supabase
    .from("opportunity_candidates")
    .update(observationUpdateRow(current, incoming, timestamp))
    .eq("id", current.id)
    .eq("user_id", userId);
  if (current.updatedAt) query = query.eq("updated_at", current.updatedAt);
