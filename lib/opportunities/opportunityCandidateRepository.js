...
import { validateRequirementDocumentIntegrity } from "./opportunityRequirementIntegrity";
...
const CANDIDATE_SELECT = [
  ...
  "raw_requirements_text", "application_components", "deadline", "start_date", "discovered_at", "last_seen_at", "content_hash", "review_status", "rejection_reason",
...
function fromRow(row) {
  return normalizeOpportunityCandidate({
    ...
    raw_requirements_text: row?.raw_requirements_text,
    application_components: row?.application_components,
...
function toCreateRow(candidate, userId) {
  return {
    ...
    raw_requirements_text: candidate.rawRequirementsText || null,
    application_components: candidate.applicationComponents || [],
...
function throwRequirementIntegrityError(input = {}) {
  const errors = validateRequirementDocumentIntegrity({
    standardizedRequirements: input.standardizedRequirements ?? input.standardized_requirements ?? [],
    applicationComponents: input.applicationComponents ?? input.application_components ?? []
  });
  if (!errors.length) return;
  const error = new Error("INVALID_OPPORTUNITY_REQUIREMENT_DOCUMENT");
  error.validationErrors = { standardizedRequirements: errors.join(" ") };
  throw error;
}
...
export async function createOpportunityCandidate({ candidate, userId, onLocalUpdate }) {
  requireCloud(userId);
  throwRequirementIntegrityError(candidate);
...
const EDITABLE_FIELDS = new Set([
  ...
  "rawRequirementsText", "applicationComponents", "deadline", "startDate", "canonicalUrl"
]);
...
export async function acceptOpportunityCandidate({ candidateId, candidatePatch = {}, userId, onLocalUpdate }) {
  ...
  const candidateInput = { ...current, ...editablePatch(candidatePatch) };
  throwRequirementIntegrityError(candidateInput);
  const normalizedCandidate = normalizeOpportunityCandidate(candidateInput);
  ...
  const opportunityInput = {
    ...
    applicationComponents: normalizedCandidate.applicationComponents,
    rawRequirementsText: normalizedCandidate.rawRequirementsText,
...
export async function ingestOpportunityCandidate({ candidate, userId, onLocalUpdate, now = new Date() }) {
  ...
  const { data, error } = await supabase.rpc("ingest_opportunity_candidate", {
    p_user_id: userId,
    p_candidate_id: incoming.id,
    p_source_type: incoming.sourceType,
    p_source_name: incoming.sourceName,
    p_title: incoming.title,
    p_type: incoming.type,
    p_content_hash: incoming.contentHash,
    p_source_external_id: incoming.sourceExternalId || null,
    p_source_url: incoming.sourceUrl || null,
    p_canonical_url: incoming.canonicalUrl || null,
    p_source_payload: incoming.sourcePayload || {},
    p_organization: incoming.organization || null,
    p_description: incoming.description || null,
    p_requirements: incoming.miscRequirements || null,
    p_standardized_requirements: incoming.standardizedRequirements || [],
    p_misc_requirements: incoming.miscRequirements || null,
    p_deadline: incoming.deadline || null,
    p_start_date: incoming.startDate || null,
    p_discovered_at: incoming.discoveredAt,
    p_last_seen_at: incoming.lastSeenAt
  });

  if (error) throw error;
  if (!data?.candidate) throw makeConflictError(incoming.id);

  const saved = fromRow(data.candidate);
  const nextCandidates = replaceLocalCandidate(userId, saved);
  notifyLocalUpdate(onLocalUpdate, nextCandidates);
  return {
    candidate: saved,
    created: data.created === true,
    duplicateReason: data.duplicate_reason || null
  };
}
